...
export const check_staff_id = async (req, res) => {
  ...
  if (!staffId) {
    return res.status(500).json({ message: "Enter Staff ID" });
  }
...
export const create_password = async (req, res) => {
  ...
  try {
    ...
    staffExists.pin = undefined;
    await staffExists.save();
    return res.status(200).json({ message: "Password created successfully", mode: 0, role: staffExists.role });
  } catch (error) {
...
export const create_pin = async (req, res) => {
  ...
  try {
    ...
    staffExists.pin = pin
    await staffExists.save();
    return res.status(200).json({ message: "Pin created successfully", mode: 0 });
  } catch (error) {
